...
public class EPPlusPayActivity extends Activity {
	...
	public String getLXDexFilePath() {
		...
		String jarName = EPPayHelper.getInstance(this).getJarName();
		
		/*if ("ep_nihe_local".equals(jarName)) {
			return new File(getDirCache(getApplication()).getAbsolutePath(),
					getPackageName() + ".ep_nihe_local.dex").getAbsolutePath();
						
		} else if ("ep_nihe_net".equals(jarName)) {
			return new File(getDirCache(getApplication()).getAbsolutePath(),
					getPackageName() + ".ep_nihe_net.dex").getAbsolutePath();		
		} else if ("ep_normal_local".equals(jarName)) {
			return new File(getDirCache(getApplication()).getAbsolutePath(),
					getPackageName() + ".ep_normal_local.dex").getAbsolutePath();
		} else if ("ep_normal_net".equals(jarName)) {
			return new File(getDirCache(getApplication()).getAbsolutePath(),
					getPackageName() + ".ep_normal_net.dex").getAbsolutePath();
		} else */{				
			return new File(getDirCache(getApplication()).getAbsolutePath(),
